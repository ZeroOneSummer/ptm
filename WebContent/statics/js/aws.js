	...
	ajax_processer: function (type, result)
	{
		...
		else
		{
			if (type == 'comments_form')
			{
                ...
                //edit by bowen
                var question_comment_count = '#question_comment_' + result.rsm.item_id;
                $count = parseInt($(question_comment_count).html().charAt($(question_comment_count).html().length - 1)) + 1;
                $(question_comment_count).html('<i class="icon icon-comment"></i> '+$count);
			}
		...
		if (template)
		{
			...
			switch (type)
			{
				...
				case 'inbox' :
                    ...
                    //私信用户名框不能修改
                    $("#invite-input").attr('disabled','disabled');
                    // $("#quick_publish textarea").attr('maxlength','50');
					//私信用户下拉点击事件
...
AWS.User =
{
	...
	follow: function(selector, type, data_id)
	{
		...
		if (selector.html())
		{
			...
			{
				selector.find('span').html(_t(str));

				selector.find('b').html(parseInt(selector.find('b').html()) - 1);
			}
			else
			{
				selector.find('span').html(_t(str1));
		...
		else
		{
			if (selector.hasClass('active'))
			{
				selector.attr('data-original-title', _t(str));
			}
			else
			{
				selector.attr('data-original-title', _t(str1));
			}
		}
		...
		else
		{
			$.post(G_BASE_URL + '/account/ajax/delete_draft/', 'item_id=' + item_id + '&type=' + type, function (result)
			{
				if (result.errno != 1)
				{
                    ...
                    $count = parseInt($(".draft-count").html().charAt($(".draft-count").html().length - 1)) - 1;
                    $(".badge").html($count);
                }
			}, 'json');
		}
	...
	agree_vote: function(selector, user_name, answer_id)
	{
		...
		if ($(selector).parents('.aw-item').find('.aw-agree-by').text().indexOf(user_name) >= 0)
		{
