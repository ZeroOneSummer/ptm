 var _hmt = _hmt || [];
        ...
        $("#vouchers_center_header > a").click(function(){
            vouchers_type = $(this).attr("vouchers_type");
            if(vouchers_type != undefined){
                $(this).addClass("btn_fd5353 fff");
                $("#vouchers_center_header > a").not( $(this) ).removeClass("btn_fd5353 fff");
                getVouchersList(page);
            }
        });

        function getVouchersList(page){
            var url2 = "https://deposit.koudailc.com/voucher/show-vouchers?clientType=pc&page="+page+"&pageSize="+pageSize+"&state="+vouchers_type;
            var html2 ='';
            var params =  {};
            KD.util.post(url2, params, function(result){
                if( result.code == 0){
                    ...
                    html2 +='<td class="a_center">口袋券</td>';
                    html2 +='<td>适用范围</td>';
                    html2 +='<td class="a_center">有效期至</td>';
                    html2 +='</tr>';
                    if (result.data.length == 0) {
                        ...
                        html2 +='</tr>';
                        html2 +='</table>';
                        $('#vouchers_list').html(html2);
                        $('#vouchers_list_page').html('');
                        return false;
                    }
                    $.each(result.data,function(index,value){
                        html2 +='<tr class="_666 f_14_16">';
                        if( value.deduction.length >= 3 ){
                            f_size = 'f_20_22';
                        }else{
                            f_size = 'f_26_28';
                        }
                        var img_index = value.type == 4 ? 2 : value.type ;
                        if( vouchers_type == 2 ){
                            if( value.type == 1 || value.type == 3 || value.type == 4 ){
                            html2 +='<td class="a_left fff '+f_size+'"><p style="padding-left:42px;width:54px;height:63px;line-height:63px;background: url(https://www.koudailc.com/image/site/account/vouchers/fringe'+img_index+'_1.png) no-repeat center center;background-size:100%;"><span class="f_16">￥</span>'+value.deduction+'</p></td>';
                            }else{
                            html2 +='<td class="a_left fff '+f_size+'"><p style="padding-left:42px;width:54px;height:63px;line-height:63px;background: url(https://www.koudailc.com/image/site/account/vouchers/fringe'+img_index+'_1.png) no-repeat center center;background-size:100%;">'+value.deduction+'</p></td>';
                            }
                        }else{
                            if( value.type == 1 || value.type == 3 || value.type == 4 ){
                            html2 +='<td class="a_left fff '+f_size+'"><p style="padding-left:42px;width:54px;height:63px;line-height:63px;background: url(https://www.koudailc.com/image/site/account/vouchers/fringe'+img_index+'.png) no-repeat center center;background-size:100%;"><span class="f_16">￥</span>'+value.deduction+'</p></td>';
                            }else{
                            html2 +='<td class="a_left fff '+f_size+'"><p style="padding-left:42px;width:54px;height:63px;line-height:63px;background: url(https://www.koudailc.com/image/site/account/vouchers/fringe'+img_index+'.png) no-repeat center center;background-size:100%;">'+value.deduction+'</p></td>';
                            }
                        }
                        if( value.type == 4 ){
                            html2 +='<td>'+value.desc+'('+value.interest_desc+')'+'</td>';
                        }else{
                            html2 +='<td>'+value.desc+'</td>';
                        }
                        html2 +='<td class="a_left _999 f_14_16">'+value.expire_info+'</td>';
                        html2 +='</tr>';
                    });
                    html2 +='</table>';
                    $('#vouchers_list').html(html2);
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        data: {
                            pages: result.pages.totalCount,
                            pageSize: pageSize,
                            cur:page,
                            methodName: 'getVouchersList'
                        },
                        success: function (data) {
                            $('#vouchers_list_page').html(data);
                        }
                    });
                }
            });
      ...
      }function alertConfirm(){ showBg("detail_invest_confirm");
      }function closeConfirm(){ closeBg("detail_invest_confirm");}
